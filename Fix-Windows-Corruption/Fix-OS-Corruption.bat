...
REM Clean up any stale flag file from previous incomplete runs
if exist "%PHASEFLAG%" (
...
ECHO [step 1/3] Checking Windows Image Health
...
ECHO [step 2/3] Scanning Windows Image Health
...
echo [Step 3/3] Repairing Windows Image... >> "%LOGFILE%"
	dism /online /cleanup-image /restorehealth >> "%LOGFILE%" 2>&1
		if %errorlevel% neq 0 (
		echo ERROR: RestoreHealth encountered errors. Check log file. >> "%LOGFILE%"
		echo WARNING: DISM RestoreHealth encountered errors.
...
echo.
    ...
    if %errorlevel% equ 0 (
        ...
        shutdown /r /t 0 /c "Restarting for CHKDSK and DISM completion. Repair script will auto-resume after login."
        exit
	) else (
...
echo.
	...
	if /i "%REPAIR_APPX%"=="Y" (
		...
		PowerShell -ExecutionPolicy RemoteSigned -Command "Get-AppXPackage -AllUsers | Foreach {Add-AppxPackage -DisableDevelopmentMode -Register \"$($_.InstallLocation)\AppXManifest.xml\" -ErrorAction SilentlyContinue}" >> "%LOGFILE%" 2>&1
